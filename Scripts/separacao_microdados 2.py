...
# Verificar se diretório existe para salvar o arquivo final
...
if len(list_files) > 0:
    ...
    if filtro_setor == 'S':
        df_final.to_csv("./data/raw/" + ano + ".csv", index=False, sep=";", encoding='utf-8')
        print("Arquivo CSV salvo com sucesso: DATA_RAIS_SETORTI_SOROCABA_" + ano + ".csv")
    else:
        df_final.to_csv("./data/raw/" + ano + ".csv", index=False, sep=";", encoding='utf-8')
        print("Arquivo CSV salvo com sucesso: DATA_RAIS_GERAL_SOROCABA_" + ano + ".csv")
else:
    print("Nenhum arquivo CSV encontrado para leitura.")
